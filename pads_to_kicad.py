...
def _append_caedecal_graphics(
    ...
    # For IC* symbols, align graphics anchor to pinmap anchor so body box follows
    # original CAEDECAL position relative to pins.
    if prefer_pinmap_anchor and caedecal_def.pinmaps:
        pin_pts = [(pm.raw_x, pm.raw_y) for pm in caedecal_def.pinmaps]
        has_origin = any(px == 0 and py == 0 for px, py in pin_pts)
        if has_origin:
            anchor_x = 0.0
...
def _format_lib_symbol(
    ...
    used_caedecal = False
    # For 2-pin passives (R/C/L), procedural graphics are more stable.
